...
## 🤖 AI Agent 團隊 (12 個專業 Agents)

### 核心 Agent
1. **Coordinator** - 協調者，負責任務調度與團隊管理
2. **Product Manager** - 產品經理，負責需求分析與 PRD 撰寫
3. **Solution Architect** - 架構師，負責系統設計與技術決策

### 開發 Agent
4. **Backend Developer** - 後端工程師（API & Backend Implementation）
5. **Frontend Developer** - 前端工程師（UI Development）
6. **UI/UX Designer** - UI/UX 設計師（介面設計、原型製作、使用者旅程規劃）
7. **QA Engineer** - 測試工程師（測試計劃、自動化測試、Bug 追蹤）

### 運維 Agent
8. **DevOps Engineer** - 運維工程師（部署、監控、備份管理）
9. **Data Analyst** - 數據分析師（數據分析、報表、RAG 評估）
10. **Knowledge Manager** - 知識管理員（知識策展、內容分類、品質監控）

### 守護者 Agent (Phase 1 新增)
11. **FinOps Guardian** - 成本守護者（成本估算、資源優化、預算監控）
12. **Security Guardian** - 安全守護者（安全審查、漏洞掃描、合規檢查）
